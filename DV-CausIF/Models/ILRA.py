...
"""
from our_topk.Models.causal5 import CausalModel
...
class ILRA(nn.Module):
    def __init__(self, num_layers=2, feat_dim=1024, n_classes=1, hidden_feat=512, num_heads=8, topk=1, ln=True,
        ...
        self.causal=causal
        if causal:
            self.causal = CausalModel(q_dim=hidden_feat, k_dim=feat_dim, v_dim=hidden_feat,cat=cat)

        if cat:
            # classifier
            self.classifier = nn.Linear(in_features=hidden_feat*2, out_features=n_classes)
        else:
    ...
    def forward(self, data):
        ...
        if self.causal:
            cluster=data[1]
            feat = self.causal.forward(feat,cluster)
        logits = self.classifier(feat)

        return logits,feat,attn_softmax
